...
function FieldCard({
  ...
  return (
    <>
      ...
      >
        {/* Row 1: field name + visualization */}
        ...
        {/* "undefined" shown once per card, only when field is missing in some docs */}
        {isUndefinedInSome && (
        ...
        {/* Value distribution bar for enum-like strings */}
...
export function SchemaTab() {
  ...
  useEffect(() => {
    const cached = queryClient.getQueryData<SchemaAnalysisResult>(
      QUERY_KEYS.schema.all(dbName, collectionName),
    );
    if (cached) {
