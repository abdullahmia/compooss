...
import type { PipelineSettings as PipelineSettingsType } from "./use-aggregation-pipeline";

interface PipelineSettingsModalProps {
  open: boolean;
  settings: PipelineSettingsType;
  onClose: () => void;
  onApply: (settings: PipelineSettingsType) => void;
}
...
export function PipelineSettingsModal({
  ...
  const [local, setLocal] = useState<PipelineSettingsType>(settings);
  ...
  const update = <K extends keyof PipelineSettingsType>(
    key: K,
    value: PipelineSettingsType[K],
  ) => {
